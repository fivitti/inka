...
test(editField_ok) {
  ...
  assertTrue(equalCharArrays(buffer_, "32 "));
...
test(addField_ok_existed_row) {
  ...
  assertTrue(file.gotoBeginOfLine()); // 1 field
  assertTrue(file.nextField()); // 2 field
  assertTrue(file.nextField()); // 3 field
...
test(addField_num_ok) {
  ...
  assertTrue(file.nextField()); // 5 (new) field
  assertEqual(file.readField(buffer_, bufferSize), 4);
  assertTrue(equalCharArrays(buffer_, "7   "));
